...
var _export = {
    ...
    eth: {
        canPay: async (_account, _fee) => {
            let account = _export.config.getAccountValue(_account)
            let balance = await web3.eth.getBalancePromise(account)
            return new BigNumber(balance).gte(_fee)
        }  
    },
    network: {
